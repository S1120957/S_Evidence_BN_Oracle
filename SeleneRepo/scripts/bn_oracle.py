"""
bn_oracle.py â€” SELENE off-chain Bayesian Network inference engine.

Paper reference: SELENE: Scalable Evidence-Based Oracles Using Bayesian Networks
...
Posterior (Equation 2 in paper):
    P(PPH, PPR | e) âˆ P(PPH) P(PPR) * prod_{E_i observed} P(E_i=e_i | PPH, PPR)

Missing evidence (âŠ¥ variables) is marginalized out by omitting the
corresponding factor from the product â€” not by substituting a default value.

Fixed-point encoding (Section III-D):
    p_hat = round(p * SCALE),   p_hat in {0, â€¦, SCALE}
    |p - p_hat/SCALE| â‰¤ 1/(2*SCALE) = 5e-7  for SCALE=1_000_000
...
# ---------------------------------------------------------------------------
# Constants â€” must match CPTStore.SCALE
# ---------------------------------------------------------------------------
...
class BNOracle:
    """
    Closed-form Bayesian Network inference engine for the SELENE healthcare
    ...
    def infer(
        ...
        Implements Equation (2) from the paper:
            P(PPH, PPR | e) âˆ P(PPH) P(PPR) âˆ_{E_i observed} P(E_i=e_i | PPH, PPR)

        Missing / None / 'âŠ¥' evidence variables are marginalized out by
        ...
        evidence : mapping of evidence name -> observed value.
            ...
            Missing / unobserved: absent, None, or 'âŠ¥'.
            Example: {"GPS": 1, "PC": 0, "PMD": None, "PR": "âŠ¥"}
    ...
    def encode(p: float) -> int:
        ...
        Absolute rounding error: |p - p_hat/SCALE| â‰¤ 1/(2*SCALE) = 5e-7.
...
def _normalize_evidence(
    ...
    Missing keys, None, and 'âŠ¥' values are treated as unobserved and
    ...
    for ev_name in EVIDENCE_NAMES:
        ...
        if value is None or value == "âŠ¥":
            continue
        if value not in (0, 1, False, True):
            raise ValueError(
                f"Evidence '{ev_name}' must be 0, 1, None, or 'âŠ¥'; "
                f"got {value!r}"
            )
