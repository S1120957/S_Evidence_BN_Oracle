...
# ===========================================================================
# Ternary on-chain evidence encoding (Path A)
# ---------------------------------------------------------------------------
# The on-chain contracts encode each evidence variable as a ternary value:
#
#       0 = observed false
#       1 = observed true
#       2 = UNOBSERVED  (the symbol  in the formal model)
#
# Encoding "unobserved" explicitly - rather than collapsing it to 0 - is what
# makes the on-chain evidence log a faithful inference transcript.  If an
# unobserved variable were written as 0, an independent auditor reading the
# ledger would marginalise a different variable set than the oracle did and
# would recompute a different posterior, producing a spurious discrepancy.
#
# With ternary encoding the auditor recovers exactly the assignment the oracle
# used, so recomputation is exact.  This is the property asserted by the
# transcript-completeness invariant.
# ===========================================================================
...
def canonicalize_evidence_for_chain(
    ...
    """
    Encode an evidence assignment for on-chain submission.

    Observed variables map to 0/1; unobserved variables (absent, None, or
    '') map to UNOBSERVED (2).

    Parameters
    ----------
    evidence : mapping evidence name -> 0, 1, False, True, None, or ''.

    Returns
    -------
    Dict with keys "GPS", "PC", "PMD", "PR", each in {0, 1, 2}.

    Examples
    --------
    >>> canonicalize_evidence_for_chain({"GPS": 1, "PC": 0})
    {'GPS': 1, 'PC': 0, 'PMD': 2, 'PR': 2}
    """
    observed = _normalize_evidence(evidence)
    return {
        name: observed.get(name, UNOBSERVED)
        for name in EVIDENCE_NAMES
    }


def decode_chain_evidence(
    gps: int, pc: int, pmd: int, pr: int,
) -> Dict[str, Optional[int]]:
    """
    Decode a ternary on-chain evidence record back to an evidence mapping.

    This is the auditor-side inverse of canonicalize_evidence_for_chain().
    Feeding the result to BNOracle.infer() reproduces exactly the posterior
    the oracle committed, which is what makes on-chain verification exact.

    Parameters
    ----------
    gps, pc, pmd, pr : int in {0, 1, 2} as stored on-chain.

    Returns
    -------
    Dict mapping evidence name -> 0, 1, or None (None = unobserved).
    """
    raw = (gps, pc, pmd, pr)
    decoded: Dict[str, Optional[int]] = {}
    for name, value in zip(EVIDENCE_NAMES, raw):
        if value == UNOBSERVED:
            decoded[name] = None
        elif value in (0, 1):
            ...
            raise ValueError(
                f"On-chain evidence '{name}' must be 0, 1, or "
                f"{UNOBSERVED} (UNOBSERVED); got {value!r}"
            )
    return decoded


def observed_count(evidence: Mapping[str, Optional[Any]]) -> int:
    """Number of evidence variables actually observed (0..4)."""
    return len(_normalize_evidence(evidence))
